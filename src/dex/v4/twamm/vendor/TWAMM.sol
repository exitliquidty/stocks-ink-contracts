...
// Vendored from https://github.com/akshatmittal/v4-twamm-hook (src/TWAMM.sol), commit
// 02950985541a210244d33a4d5c8e3efad8d4de2c -- vendored with the author's direct permission (the
// upstream repo carries no public LICENSE / SPDX license, only UNLICENSED; do not re-vendor or
// redistribute further from this copy without your own separate permission). Audited twice
// against this exact commit: Certora (Jan 2025, 2 Critical findings, both fixed) and ABDK
// Consulting (23 Jan 2025, 1 Major finding, fixed; 27 other Low/Info findings) -- both reports are
// mirrored at audits/twamm/. Only the import paths, `solmate Owned` -> the already-vendored
// `@openzeppelin Ownable` swap (see that import's own comment below), and the `@src`/`@lib` remap
// aliases were changed to fit this repo's layout -- the audited logic itself is untouched, with
// two narrow exceptions, both purely additive `virtual` keywords, not behavior changes to this
// contract in isolation: `getHookPermissions()` (see its own comment) so StocksHook can extend the
// permission set, and `beforeSwap()` (see its own comment) so StocksHook can layer its own
// protocol-fee-on-input logic on top of this function's own unmodified executeTWAMMOrders() call
// and ZERO_DELTA return, via `super.beforeSwap(...)`-equivalent reasoning (StocksHook's own
// override re-runs this exact body rather than calling super, to avoid a double
// executeTWAMMOrders() call -- see that override's own comment).

import {TwammBaseHook as BaseHook} from "./TwammBaseHook.sol";
...
// Swapped for the already-vendored OpenZeppelin equivalent instead of adding solmate as a new
// dependency -- constructor(address initialOwner) and the onlyOwner modifier match exactly, and
// this is the only place solmate was used in the original.
import {Ownable as Owned} from "@openzeppelin/contracts/access/Ownable.sol";
...
contract TWAMM is BaseHook, Owned, ITWAMM {
    ...
    /// @inheritdoc BaseHook
    // `virtual` added (the only behavioral-code edit made to this vendored file -- see this file's
    // own header) so StocksHook can extend this set with afterSwap/afterSwapReturnDelta for
    // its own fee-taking, on top of every flag TWAMM itself needs. No existing override exists
    // upstream, so this changes nothing about TWAMM's own behavior in isolation.
    ...
    // `virtual` added (see this file's own header) so StocksHook can layer its own
    // protocol-fee-on-input logic on top of this exact body.
